...
namespace MaintenanceApp
{
    ...
    public sealed partial class Cleaning : Page
    {
        ...
        ObservableCollection<services> cleaningList = new ObservableCollection<services>();
        ...
        private void workList()
        {
            ...
            {
                if (s.serviceType.ToLower().Equals("cleaning"))
                {
                    cleaningList.Add(s);
                }
            }
            CleaningList.ItemsSource = cleaningList;
        }
        ...
        {
            foreach (services m in cleaningList)
            {
                if (m.serviceID == id)
                {
            ...
            using (var db = new AzureDataBase())
            {
                foreach (services s in cleaningList)
                {
                    if (s.serviceID == id)
                    {
                        db.Update(s);
